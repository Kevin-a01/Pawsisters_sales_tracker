...
function SalesDetail() {
  ...
  const filteredSales = sales.filter((sale) => {
    ...
    const payMatch = filterPayment ? pay === filterPayment.toLowerCase() : true;
  ...
  const customLabel = (entry) => {
    const maxLenght = 10;
    const name =
      entry.product.length > maxLenght
              ...
              <div key={date} className="mb-6 mt-5">
                ...
                <table className="w-full border-collapse mt-4">
                  ...
                  <tbody>
                    {salesDate.map((sale) => (
                      <tr key={sale.id}>
                        <td className="border text-sm border-pink-300 p-2 text-center">
                          {sale.product}
                        </td>
