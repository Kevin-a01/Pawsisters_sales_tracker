    ...
    const fetchDetails = async () => {
      try {
        const res = await fetch(
          `${API_BASE_URL}/api/calendar/details?date=${date}`
        );
  ...
  const toggleTask = async (taskItem) => {
    try {
      ...
      if (res.ok) {
        ...
        setTask((prev) =>
          prev.map((t) =>
            t.task_id === taskItem.task_id ? updatedTask.task : t
          )
        );
      } else {
      ...
      <div className="w-full ">
        <div className="flex justify-between">
          <button
            ...
            className="text-2xl ml-2 bg-[#F4538B] w-10 flex justify-center h-10  items-center text-white rounded-full md:mt-3 md:mr-3"
          >
            <i class="fa-solid fa-trash"></i>
          </button>

          <Link
            to={`/calendar/${date}/add-event`}
            className="text-2xl mr-2 bg-[#F4538B] w-10 flex justify-center h-10  items-center text-white rounded-full md:mt-3 md:ml-3 "
          >
        ...
        {Array.isArray(details) &&
          details.map((event) => (
            <React.Fragment key={event.id}>
              ...
              <div
                key={event.id}
                className="bg-[#FEF2F6] rounded-xl shadow-md p-4 mx-10 mt-3 flex justify-center items-center flex-col gap-1 text-[#F4538B]"
              >
                <h2 className="">Event detta datum:</h2>
              ...
              <div className="bg-[#FEF2F6] rounded-xl shadow-md p-4 mx-10 mb-10 mt-3 flex flex-col gap-1 text-[#F4538B] relative">
                <div className="w-full">
                  <Link
                    to={`/add-task/${date}`}
                    className="p-2 absolute top-[-6px] right-[-1px] text-2xl rounded-xl "
                  >
                      ...
                      >
                        <li
                          className={`text-lg cursor-pointer flex items-center gap-2 ${
                            taskItem.completed ? " text-gray-400" : ""
                          }`}
