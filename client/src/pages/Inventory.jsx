  ...
  return (
    <>
      ...
      <div className="flex pl-2 gap-1 w-full">
        ...
        <Link
          to="/add-new-item"
          className="text-lg p-1 ml-auto bg-pink-400 w-20 flex items-center justify-center mr-2 rounded-xl md:mt-2"
        >
          Add
        </Link>
      </div>
      ...
      <div className="py-3 pl-3 flex justify-between w-full ">
        <input
          className="border w-45 pl-1 rounded-xl h-9 focus:outline-none border-purple-500"
          type="text"
        ...
        <div className="p-2 relative w-22 mr-1">
          <Listbox value={selected} onChange={setSelected}>
            <ListboxButton className="w-full bg-pink-700 text-white  rounded-lg shadow hover:bg-pink-800 transition focus:outline-none p-1">
              {selected}
            </ListboxButton>
            <ListboxOptions
              anchor="bottom"
              className="absolute mt-1 bg-white w-20 text-center rounded-xl font-medium"
            >
              {options.map((option) => (
                <ListboxOption
                  key={option}
                  value={option}
                  className="data-focus:bg-[#FCD4DF] cursor-pointer border-none"
                >
                  {option}
                </ListboxOption>
              ))}
        ...
        {filteredProducts.length === 0 ? (
          ...
          Object.entries(groupedByMaker).map(([maker, makerProduct]) => (
            <div key={maker} className="mb-10">
              <h2 className="text-center text-3xl bg-pink-300 w-1/2 mx-auto p-2 rounded-2xl pb-2 mb-4 md:w-1/3">
                {maker}
              </h2>
              ...
              {makerProduct.map((product) => (
                <div
                  key={product.id}
                  className="p-2 bg-[#FCD4DF] rounded-2xl flex flex-col gap-2 items-start mb-5"
                >
                  ...
                  {editingProductId === product.id ? (
                    ...
                    <>
                      <h3 className="p-1 font-medium text-lg">
                  ...
                  <img
                    className="w-20 rounded-2xl mx-2 ml-auto -mt-20 "
                    src={product.image}
                    alt={product.name}
                  />
                </div>
