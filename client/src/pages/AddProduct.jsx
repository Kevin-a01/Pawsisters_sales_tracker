...
function AddProduct() {
  ...
  const handleCheckbox = (e) => {
    setForm({
      ...form,
      payment: { ...form.payment, [e.target.name]: e.target.checked },
    });
  };
  ...
  const addProduct = async (e) => {
    ...
    try {
      ...
      navigate("/sales-tracker", {
        state: { conId: currentConId, conTitle: currentConTitle },
      });
    } catch (error) {
    ...
    <>
      ...
      {loading ? (
        ...
        <div className="p-3">
          <form
            onSubmit={addProduct}
            className="space-y-5 flex flex-col justify-center items-center pt-2 pb-2"
          >
            <div className="w-11/12 lg:w-1/4">
              <label className="text-lg font-medium text-pink-500">
                Lägg till produkt och skapa nytt konvent{" "}
              </label>
              <select
                name=""
                id=""
              ...
              >
                ...
                <option value="new">Skapa Nytt Konvent</option>
              </select>
            </div>
            {showNewConInput && (
              <input
