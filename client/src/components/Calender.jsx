...
export default function Calender() {
  ...
  useEffect(() => {
    ...
    const fetchEvents = async () => {
      const res = await fetch(
        `${API_BASE_URL}/api/calendar/events?start=${start}&end=${end}`
      );
    ...
    const fetchMonthlyNote = async () => {
      const res = await fetch(
        `${API_BASE_URL}/api/calendar/monthly-note?year=${currentWeekStart.year()}&month=${
          currentWeekStart.month() + 1
        }`
      );
      const data = await res.json();
  ...
  const weekText = `V. ${weekNumber}  ${monthName} - ${currentWeekStart.format(
    "YYYY"
  )}`;
  ...
  return (
    <>
      <div className="max-w-4xl mx-auto px-4 py-5">
        <div className="flex items-center justify-between mb-4">
          <button
            ...
            className="bg-[#F4538B] text-white px-3 py-1 rounded-xl hover:bg-pink-600/50"
          >
            <i className="fa-solid fa-arrow-left"></i>
          </button>
          <h2 className="text-2xl font-medium text-center">{weekText}</h2>
          <button
            onClick={() => handleChangeWeek(1)}
            className="bg-[#F4538B] text-white px-3 py-1 rounded-xl hover:bg-pink-600/50"
          >
      ...
      <div className="mb-4 p-5 m-3 bg-[#FCD4DF] border border-pink-300 rounded-2xl shadow-sm shadow-[#F4538B] md:w-1/2 md:mx-auto">
        <div className="flex justify-between">
          <h3 className="font-bold text-xl pb-2 ">Veckonotis!</h3>
        </div>

        {events.filter((event) =>
          dayjs(event.date).isSame(currentWeekStart, "week")
        ).length > 0 ? (
          <ul className="mt-3 md:text-xl list-disc list-inside text-[#F4538B] font-medium">
            {events
              .filter((event) =>
                dayjs(event.date).isSame(currentWeekStart, "week")
              )
      ...
      <div
        ref={scrollRef}
        className="m-3 overflow-x-auto md:flex md:justify-center "
      >
        <div className="flex gap-2 min-w-[700px] m-2 p-2">
          {daysofWeek.map((day) => {
            ...
            const dayName =
              ...
              day.format("ddd").slice(1);
            const formattedDate = day.format("D MMMM");

            return (
              <Link
                key={day.format("YYYY-MM-DD")}
                to={`/calendar/${day.format("YYYY-MM-DD")}`}
              >
                <div
                  className={`p-2 w-45 shrink-0 border rounded-xl flex flex-col justify-between sm:justify-start  cursor-pointer h-36 realtive
                    ...
                    {events
                      ...
                      .map((event) => (
                        <div key={event.id} className="text-md">
                          {event.title}
                          <p className="text-sm pt-4">{event.description}</p>
                        </div>
                      ))}
                  </div>
                </div>
